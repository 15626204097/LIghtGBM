# -*- coding: utf-8 -*-
"""
Created on Fri Jun 22 20:28:25 2018
...
# 加载数据
row_data = pd.read_csv('/Users/a/Desktop/P2P/test280014.csv',header=None)#如果文件中没有列名，则默认为0，否则设置为None。
...
test_RMSE=np.zeros(100)

for num in range(0,100):
    y_pred = gbm.predict(X_test, num_iteration=num)#迭代寻优了   
    RMSE=mean_squared_error(y_test, y_pred) ** 0.5
    test_RMSE[num]=RMSE
             
             
train_RMSE=np.zeros(100)
for num in range(0,100):
    y_pred = gbm.predict(X_train, num_iteration=num)#迭代寻优了   

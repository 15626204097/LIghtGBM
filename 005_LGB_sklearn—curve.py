...
def plot_learning_curve(estimator, title, X, y, ylim=None,   
                     train_sizes = [100,500,1000,1500,1886],cv=10):   #
    """     画出data在某模型上的learning curve.     
    参数解释     ----------     
    estimator : 你用的分类器。表我们估计实际模型时所用的学习算法；    
    title : 表格的标题。    
    X : 输入的feature，numpy类型   ,包含特征的数据；
    y : 输入的target vector     ,包含目标的数据；
    ...
    train_scores:训练集上准确率
     test_scores:交叉验证集上的准确率
      ...
      我们将会使用 80:20 的比例来设置训练集和验证集，最终我们的训练集有 7654 个样本（80%），验证集有 1914 个样本（20%），用来生成学习曲线的训练集的最大样本数就是 7654。
 
      在这种情况下，我们用以下 6 种大小的训练集：
      train_sizes = [1,100,500,2000,5000,7654] 6份每份比例
     cv-确定交叉验证分割策略（我们马上会讨论这个内容）；
     scoring-代表所用的误差指标；我们使用 nearest proxy 和负 MSE，我们随后必须颠倒一下符号。
     scoring = 'neg_mean_squared_error',
    """    
    plt.figure()    
    train_sizes, train_scores, test_scores = learning_curve(  estimator, X, y,  n_jobs=1,  train_sizes = [100,500,1000,1500,1886],cv=10)    
    ...
    plt.title(title)   
    plt.show()
    #train_sizes=np.linspace(.05, 1., 20),cv=10)  train_sizes=np.linspace(.25, 1, 10)
plot_learning_curve(estimator, "LGBM", X, y, ylim=(0.8, 1.05), train_sizes = [100,500,1000,1500,1886],cv=10)
